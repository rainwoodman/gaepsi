...
static inline fckey_t _truncate(fckey_t key, int order) {
  return (key >> (order * 3)) << (order * 3);
}

static inline void _flatten(fckey_t key, ipos_t *x, ipos_t * y) {
  *x = 0; *y = 0;
  int base = 0;
  while(key >0) {
    *x += (key & 0x7) << base;
...
static inline fckey_t _xyz2ind (ipos_t x, ipos_t y, ipos_t z) {
    fckey_t ind = 0;
    x &= (1L << BITS) - 1;
    y &= (1L << BITS) - 1;
    z &= (1L << BITS) - 1;
    int i;
    for (i = 0; i < BITS; i+= 8) {
      ind |= (fckey_t) utab[(uint8_t) x] << (i*3+0);
      ind |= (fckey_t) utab[(uint8_t) y] << (i*3+1);
      ind |= (fckey_t) utab[(uint8_t) z] << (i*3+2);
      x >>= 8;
      y >>= 8;
      z >>= 8;
    }
...
fckey_t masks[] = {
     ((fckey_t)0111111111111111111111L << 63) | ((fckey_t)0111111111111111111111L),
     ((fckey_t)0222222222222222222222L << 63) | ((fckey_t)0222222222222222222222L),
     ((fckey_t)0444444444444444444444L << 63) | ((fckey_t)0444444444444444444444L),
};

static ipos_t inline _ind2x(fckey_t ind) {
    fckey_t comp = ind & masks[0];
    ipos_t x = 0;
    uint8_t raw;
...
void inline _ind2xyz(fckey_t ind, ipos_t * x, ipos_t * y, ipos_t * z) {
  *x = _ind2x(ind);
  *y = _ind2x(ind>>1);
  *z = _ind2x(ind>>2);
}

static int _boxtest(fckey_t key, int order, fckey_t point) {
  return 0 == ((key ^ point) >> (order * 3));
}

void inline _diff(fckey_t ind1, fckey_t ind2, ipos_t d[3]) {
  d[0] = _ind2x(ind2);
  d[1] = _ind2x(ind2>>1);
  d[2] = _ind2x(ind2>>2);
  d[0] -= _ind2x(ind1);
...
int inline _diff_order(fckey_t ind1, fckey_t ind2) {
  fckey_t diff = ind2 ^ ind1;
  int r = 0;
...
static char * _format_key(fckey_t key) {
  static char buf[100];
  ipos_t ix, iy, iz;
...
static int _AABBtest(fckey_t key, int order, fckey_t AABB[2]) {
  /**** returns -2 if key, order fully in AABB, returns -1 if partially in,
 *      0 if not in  ***/
  int d, i;
  ufckey_t l, x0, x1, r;
  int in = 0;
  ufckey_t m = ((fckey_t)1 << (3*order)) - 1;
  for(d=0; d<3; d++) {
    l = (key & masks[d]) & ~m;
    r = l + (masks[d] & m);
    x0 = AABB[0] & masks[d];
...
int main() {
    ...
    for(i = 1; i < 30; i++) {
      ...
      fckey_t ind = _xyz2ind(ix, iy, iz);
      int bit = 1 << (i-1);
      fckey_t ind2 = _xyz2ind(ix + bit, iy + bit, iz + bit);
...
static int compare_zorder(fckey_t *v, fckey_t *u, void *NOT_USED) {
    return (*v > *u) - (*v < *u);
}

#define INTP_SWAP(a, b) {intptr_t tmp = (b); (b) = (a); (a) = tmp;}
static int
aquicksort_zorder(fckey_t *v, intptr_t* tosort, intptr_t num, void *NOT_USED)
{
    fckey_t vp;
    intptr_t *pl, *pr;
